...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        pos_emb = self.transformer.wpe(pos) # (T,  ) -> (T, block_size)
        tok_emb = self.transformer.wte(idx) # (B, T) -> (B, T, vocab_size)
        x = pos_emb + tok_emb
...
class DataLoaderLite:
    ...
    def next_batch(self):
        ...
        self.current_pos += B * T * self.num_process
        if self.current_pos + (B * T * self.process_rank + 1) > len(self.tokens):
            self.current_pos = self.B * self.T * self.process_rank
        return x, y
